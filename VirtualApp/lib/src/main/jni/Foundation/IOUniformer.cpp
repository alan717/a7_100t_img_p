...
HOOK_DEF(void*, dlopen, const char *filename, int flag) {
    ...
    ALOGD("dlopen_orig : %s, return : %p.", redirect_path, ret);
...
HOOK_DEF(void*, do_dlopen_V19, const char *filename, int flag, const void *extinfo) {
    int res;
    ALOGD("do_dlopen_v19 input_path --->: %s, return :  .", filename );
    const char *redirect_path = relocate_path(filename, &res);
    ALOGD("output_path -->:%s",redirect_path);
...
__END_DECLS
// end IO DEF
HOOK_DEF(int,luaL_loadstring,int a1,const char *a2)
{
    ALOGD("_>>>>>>luaL_loadstring:%s",a2);
    return orig_luaL_loadstring(a1,a2);
}

void onSoLoaded(const char *name, void *handle) {


    ALOGD("Soloaded :%s",name);


    if(strstr(name,"libgame.so"))
    {
        ALOGD("SO FILE-->:%s",name);
        HOOK_SYMBOL(handle,luaL_loadstring);
    }
}
