...
bool DnstNameMatches(CString HostName, PCWSTR pDNSName)
{
...
bool MatchCertificateName(PCCERT_CONTEXT pCertContext, LPCWSTR pszRequiredName) {
      ...
      pExtension->Value.pbData, pExtension->Value.cbData, 0, 0, &cbStructInfo))
   {
      ...
      {
         return Entry.dwAltNameChoice == CERT_ALT_NAME_DNS_NAME && DnstNameMatches(RequiredName, Entry.pwszDNSName);
      }
      );
   ...
   return DnstNameMatches(RequiredName, CommonName);
}
