...
{
	 std::string tid="   \n\r\t12350 ";
	 LOGD("%c %d",tid.c_str(),tid.length());
    //char url[PATH_MAX];
    //sprintf(url, "http://47.98.122.115:9090/update?game=dwrg&deviceId=%s", tid.c_str());
    std::string url("http://47.98.122.115:9090/update?game=dwrg&deviceId=");
	tid.erase(0,tid.find_first_not_of(" \n\r\t"));
	tid.erase(tid.find_last_not_of(" \n\r\t")+1);
	url.append(tid.c_str());
	LOGD(" %d",tid.length());
	LOGD("HearBeat send:%s", url.c_str());
