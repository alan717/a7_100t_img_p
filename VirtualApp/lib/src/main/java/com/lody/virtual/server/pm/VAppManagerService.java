...
public class VAppManagerService implements IAppManager {
    ...
    public InstallResult installPackage(String path, int flags) {
        return installPackage(path, flags, true);//TODO:02 INSTALL  STAGE
    }

    public synchronized InstallResult installPackage(String path, int flags, boolean notify) {
        //TODO:03 INSTALL STAGE
        //这里才是进行app的具体安装操作，现在注释开始
        long installTime = System.currentTimeMillis();//时间毫秒级
        Log.d(TAG, "installPackage: MARK+D"+path);//打印安装路径
        //实际的输出为/data/app/com.netease.mhxyhtb-1/base.apk//mhxy
        // other app --->/data/app/com.xiaomi.apps.videodaily-1/base.apk
        if (path == null) {
        ...
        //垃圾以为所有的代码都是arm直接吗？
        Log.d(TAG, "installPackage: MARK+D"+pkg.packageName);

        File libDir = new File(appDir, "lib");
        Log.d(TAG, "installPackage: MARK+D"+appDir.getName());
        Log.d(TAG, "installPackage: MARK+D"+libDir.getName());
        if (res.isUpdate) {
        ...
        res.isSuccess = true;
        Log.d(TAG, "installPackage: MARK+D IT'S TRUE");
        return res;
    }
